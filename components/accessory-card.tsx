    ...
    >
      ...
      <div className={`flex-1 ${layout === "list" ? "md:py-2" : "pt-3"}`}>
        <Link href={`/accessories/${accessory.slug}`}>
          <h3 className="font-medium text-gray-900 mb-1 line-clamp-2 hover:text-blue-600">{accessory.name}</h3>
        </Link>

        <div className="flex items-center gap-2 mb-2">
          <span className="text-gray-500 text-sm">SKU: {accessory.sku}</span>
        </div>

        <div className="flex flex-wrap gap-2 mb-3">
          {accessory.categories.slice(0, 2).map((category, index) => (
        ...
        <div className="flex items-center justify-between">
          <p className="text-lg font-bold text-gray-900">₹{accessory.price.toLocaleString()}</p>

          <div className="flex gap-2">
            {accessory.outOfStock ? (
              <a
                href={`https://wa.me/?text=${encodeURIComponent(`Hi! I'm interested in the accessory: ${accessory.name}. Please notify me when it's back in stock.`)}`}
                target="_blank"
            ...
            <button
              onClick={() => {
                const url = `${window.location.origin}/accessories/${accessory.slug}`
                if (navigator.share) {
                  navigator
                    .share({
                      title: accessory.name,
                      text: `Check out this ${accessory.name} from RC MEGA!`,
                      url: url,
                    })
                    .catch(() => {
